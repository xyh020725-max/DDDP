...
class Diffusion(object):
    ...
    def sample(self, simplified):
        ...
        if self.config.model.type == 'simple':
        ...
        elif self.config.model.type == 'icsa':
            print("===== 正在加载自定义的自适应全能压缩感知网络 (ICSA-Net) =====")
            model = ICSA_Net(self.config)
            model.to(self.device)
            
            # ======== 【修改部分开始：精准加载 CelebA-HQ 权重】 ========
            # 1. 确保下载并找到 CelebA-HQ 的预训练权重
            ckpt = os.path.join(self.args.exp, "logs/celeba/celeba_hq.ckpt")
            if not os.path.exists(ckpt):
                print(f"Downloading celeba_hq.ckpt to {ckpt}...")
                download('https://image-editing-test-12345.s3-us-west-2.amazonaws.com/checkpoints/celeba_hq.ckpt', ckpt)
            
            # 2. 读取官方权重字典
            print(f"Loading Pretrained U-Net weights from {ckpt}...")
            pretrained_state_dict = torch.load(ckpt, map_location=self.device)
            
            # 3. 强行把权重注入到你的子模块 self.unet 中
            model.unet.load_state_dict(pretrained_state_dict, strict=True)
            print("===== U-Net 预训练权重加载成功！=====")
            # ======== 【修改部分结束】 ========
            ...
            model = torch.nn.DataParallel(model)
            cls_fn = None # Inpainting 任务不需要分类器引导
            ...
            with torch.no_grad():
                ...
                for i, j in tqdm.tqdm(time_pairs):
                    ...
                    if j < i: # normal sampling 
                        ...
                        # 把当前的已知观测图像 Apy 传给 kwargs，让你的双域编码器提取特征！
                        # Apy 在外层循环 (第250行) 已经算好了: Apy = Ap(y)
                        et = model(xt, t, Apy=Apy.to('cuda'))
                        if et.size(1) == 6:
                            et = et[:, :3]
