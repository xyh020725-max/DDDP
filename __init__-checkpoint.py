...
def get_dataset(args, config):
    ...
    if config.data.dataset == "CELEBA":
        ...
        if config.data.out_of_dist:
            ...
            # =========================================================
            # 【核心修改区】统一指向我们刚刚创建的规范化数据集根目录
            # =========================================================
            standard_data_root = os.path.join(args.exp, "datasets", "celeba_hq_standard")
            
            # 1. 加载训练集 (自动带有随机水平翻转的数据增强)
            dataset = torchvision.datasets.ImageFolder(
                os.path.join(standard_data_root, "train"),
                transform=transforms.Compose([
                    transforms.Resize([config.data.image_size, config.data.image_size]),
                    transforms.RandomHorizontalFlip(p=0.5), # 训练集加入数据增强
                    transforms.ToTensor()
                ])
            )
            
            # 2. 加载验证集/测试集 (绝对不能有随机翻转，保证每次验证结果一致)
            test_dataset = torchvision.datasets.ImageFolder(
                os.path.join(standard_data_root, "val"), 
                transform=transforms.Compose([
                    transforms.Resize([config.data.image_size, config.data.image_size]),
                    transforms.ToTensor()
                ])
